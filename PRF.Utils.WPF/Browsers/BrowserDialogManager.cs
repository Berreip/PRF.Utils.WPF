...
namespace PRF.Utils.WPF.Browsers
{
    /// <summary>
    ...
    public static class BrowserDialogManager
    {
        ...
        {
            using (var ofd = new OpenFileDialog {Filter = filter, Title = title, InitialDirectory = initialDirectory, Multiselect = false})
            {
                return ofd.ShowDialog() == DialogResult.OK && File.Exists(ofd.FileName)
